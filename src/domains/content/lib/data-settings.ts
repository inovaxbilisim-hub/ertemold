...
import { mergeSectionVisibility, mergeUiContent } from '@/domains/content/lib/ui-content';
import { logSize, toBoolean } from '@/core/database/data-utils';

export type CoreSettings = Omit<SiteSettings, 'branches'>;

const getCoreSettings = cache(unstable_cache(async (): Promise<CoreSettings | null> => {
...
export const getAppearanceTheme = cache(unstable_cache(async (): Promise<string> => {
  const row = await dbGet<Record<string, unknown>>(
    `SELECT * FROM site_settings LIMIT 1`
  );
...
export const getBranches = cache(unstable_cache(async (): Promise<Branch[]> => {
...
export const getLegalPages = cache(unstable_cache(async (): Promise<Record<string, LegalPage>> => {
...
export const getStats = cache(unstable_cache(async (): Promise<Stat[]> => {
  const rows = await dbAll<Record<string, unknown>>(
...
export const getSeoByPage = cache(async (pageKey: string): Promise<SeoData | null> => {
...
export const getLocations = cache(unstable_cache(async () => {
  const rows = await dbAll<Record<string, unknown>>(
...
export const getSectionContent = cache(unstable_cache(async (): Promise<Record<string, SectionContent>> => {
  const rows = await dbAll<Record<string, unknown>>(
