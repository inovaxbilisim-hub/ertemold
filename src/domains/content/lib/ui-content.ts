...
const defaultUiContent: SiteUiContent = {
  navbar: {
    megaMenuBadge: '',
    megaMenuTitle: '',
    megaMenuDescription: '',
...
const defaultSectionVisibility: SiteSectionVisibility = {
  serviceDetail: {
    hero: true,
    content: true,
    features: true,
...
function formatUiText(template: string | undefined, values: Record<string, string | number>) {
  if (!template) {
    return '';
  }
...
function replacePlaceholders(obj: any, values: Record<string, string>): any {
